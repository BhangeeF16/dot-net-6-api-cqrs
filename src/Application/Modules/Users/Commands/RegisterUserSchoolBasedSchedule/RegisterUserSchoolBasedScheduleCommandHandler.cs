...
public class RegisterUserSchoolBasedScheduleCommandHandler : IRequestHandler<RegisterUserSchoolBasedScheduleCommand, string>
{
    ...
    public RegisterUserSchoolBasedScheduleCommandHandler(IMapper mapper,
    ...
    public async Task<string> Handle(RegisterUserSchoolBasedScheduleCommand request, CancellationToken cancellationToken)
    {
        try
        {
            #region VALIDATIONS
            ...
            var schedule = await _unitOfWork.Schools.DeliverySchedules.GetFirstOrDefaultAsync(x => x.fk_DayID == request.fk_DayID &&
                                                                                                   x.fk_TimeSlotID == request.fk_TimeSlotID &&
                                                                                                   x.fk_SchoolID == request.fk_SchoolID,
                                                                                                x => x.Day,
                                                                                                x => x.Carrier,
                                                                                                x => x.TimeSlot,
                                                                                                x => x.School,
                                                                                                x => x.School.PostCode,
                                                                                                x => x.School.PostCode.State) ?? throw new ClientException("Delivery Schedule Does not Exist", System.Net.HttpStatusCode.BadRequest);
            var school = schedule.School;
            var postCode = school.PostCode.PostalCode;
            var state = schedule.School.PostCode.fk_StateID;
            ...
            var deliverycharges = _chargeBeeService.GetSchoolDeliveryCharges(schedule, request.PlanVariation.Frequency);
            ...
            {
                new KeyValuePair<string, object>("cf_school_id", school.ID),
                new KeyValuePair<string, object>("cf_next_school_id", school.ID),
            ...
            var createSubscription = new ChargeBeeSubscription
            {
                ...
                Shipping = _mapper.Map<ChargeBeeAddress>(school),
            ...
            var dbSubscription = new UserSubscription()
            {
                ...
                IsActive = true,
                Current = new SubscriptionSetting(DeliveryScheduleType.SchoolDelivery, schedule.ID)
                {
            ...
            if (request.PlanVariation.Frequency != ShippingFrequency.OneOff)
            {
                ...
                dbSubscription.Next = new SubscriptionSetting(DeliveryScheduleType.SchoolDelivery, schedule.ID)
                {
